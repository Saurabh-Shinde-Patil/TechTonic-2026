...
const podiumConfig = {
  ...
  second: {
    bg: 'from-slate-400/20 via-gray-300/10 to-slate-500/20',
  ...
  third: {
    bg: 'from-orange-600/20 via-amber-700/10 to-orange-700/20',
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center mt-4 gap-2">
        ...
        {/* Team Name */}
        {winner.teamName && (
          <h4 className={`font-orbitron font-bold text-base md:text-lg ${style.text} text-center leading-tight`}>
            {winner.teamName}
          </h4>
        )}
        ...
        {winner.members && winner.members.length > 0 && (
          <div className="mt-2 w-full">
            <div className="flex items-center justify-center gap-1 mb-1.5">
              <Users size={12} className="text-slate-500 dark:text-gray-600" />
              <span className="text-[10px] text-slate-400 dark:text-gray-600 uppercase tracking-widest font-bold">
                {winner.members.length > 1 ? 'Team Members' : 'Participant'}
              </span>
            </div>
            <div className="space-y-1">
              {winner.members.map((member, i) => (
                <p key={i} className="text-xs text-slate-600 dark:text-gray-300 text-center font-medium">
...
function EventWinnerSection({ event, index }) {
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <div className="relative p-8 md:p-10 rounded-3xl bg-white/50 dark:bg-white/5 border border-slate-200/50 dark:border-white/10 backdrop-blur-sm hover:border-slate-300 dark:hover:border-white/20 transition-all duration-500 shadow-lg hover:shadow-xl">
